...
contract('KYCController', (accounts) => {
    let sut;
    ...
    it('unrelated accounts should not be able to add new verifiers', async () => {
        ...
        it('verifiers should be able to add new verifiers', async () => {
            const result = await sut.addKYCVerifier(verifier, {from: deployer});

            truffleAssert.eventEmitted(result, 'KYCVerifierAdded', (ev) => {
                return ev.account === verifier;
            });

            const status = await sut.isKYCVerifier(verifier);
