...
contract('ProxyToken', function ([deployer, initialHolder, recipient, votingOfficial, anotherAccount]) {
    ...
    beforeEach(async function () {
        ...
        await this.kycController.addAddressToWhitelist(deployer);
        await this.kycController.addAddressToWhitelist(votingOfficial);

        //create SUT
        this.transferQueues = await TransferQueues.new();
        this.controller = await Controller.new(this.kycController.address, this.insiderListMock.address, this.pepListMock.address);
        //TODO real kycController is here
        ...
        this.proxyToken = await VotingToken.at(this.proxy.address);

        console.log("deployer: ");
        console.log(deployer);

        console.log("votingOfficial: ");
        console.log(votingOfficial);

        console.log("proxy: ");
        console.log(this.proxy.address);


        await this.token.addVotingOfficial(this.proxyToken.address); //todo remove again
    ...
    describe('createBallot', function () {
        /*describe('when the ballotname is empty', function () {
            it('reverts', async function () {
                ...
                await expectRevert(this.proxyToken.createBallot( abi.rawEncode(['bytes32'],['']), [abi.rawEncode(['bytes32'],[''])],{from: votingOfficial}),
        ...
        describe('when the optionNames parameter is empty', function () {
            it('reverts', async function () {
                ...
                await expectRevert(this.proxyToken.createBallot( abi.rawEncode(['bytes32'],['Vote']), [],{from: votingOfficial}),
        ...
        describe('when all parameters are correct', function () {
            it('creates a ballot and stores it', async function () {
                ...
                //await this.proxyToken.addVotingOfficial(votingOfficial, {from: deployer});

                console.log("check:");
                console.log(await this.token.isVotingOfficial(deployer));
                console.log(await this.token.isVotingOfficial(votingOfficial));
                console.log(await this.proxyToken.isVotingOfficial(deployer));
                console.log(await this.proxyToken.isVotingOfficial(votingOfficial));
                console.log("check:");
                //console.log(await this.token.controller.call().then(function(v) {console.log(v.kycController);}));
                await this.token.issue(deployer, 100, abi.rawEncode(['bytes'],['']));

                await this.proxyToken.transferWithData( votingOfficial,1,abi.rawEncode(['bytes'],['']),{from: deployer});


                 //console.log(await this.proxyToken.controller.kycController._onWhitelist(deployer));
                // const { logs } =  await this.proxyToken.createBallot( abi.rawEncode(['bytes32'],['Vote']), [abi.rawEncode(['bytes32'],['A']),abi.rawEncode(['bytes32'],['B'])],{from: votingOfficial});
               //
               //  assert((await this.proxyToken.ballots(0)).name!=0);
               //  expectEvent.inLogs(logs, 'BallotCreated', { ballotName: '0x566f746500000000000000000000000000000000000000000000000000000000' });
                //assert(false); //TODO to show events
            });
        });
